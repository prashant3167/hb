...
//                        console.log(topic_list[index]['rule'][index2]['sensor'][0]);                   
...
//                       document.body.insertAdjacentHTML('beforeend',"<li>"+full_topic_name+Iagent_count+"</li>");
                    }
        }
}
    if (typeof playbook_names !== 'undefined' && playbook_names.length > 0){
...
//            console.log(count_of_iagent);
            if(count_of_iagent!=0)
                {
        option_table_entry="<tr style=\"vertical-align: middle;\"><td style=\"width: 15%;text-align:center;vertical-align: middle;\"><input type=\"checkbox\"  name=\"playbook_selected\"       id=\"option_cus"+playbook_names[index]['playbook-name']+"\" value="+playbook_names[index]['playbook-name']+" /></td><td style=\"width: 60%;\"><label for=\"option_cus"+playbook_names[index]['playbook-name']+"\">"+playbook_names[index]['playbook-name']+"</label></td><td style=\"width: 25%;text-align: center\">"
        +"<input type=\"text\" name=\"Iagent_count\"  value=\""+count_of_iagent+"\" size=\"6\"></td></tr>";
                }
            else{
                  option_table_entry="<tr style=\"vertical-align: middle;\"><td style=\"width: 15%;text-align:center;\"><input type=\"checkbox\"  name=\"playbook_selected\"       id=\"option_cus"+playbook_names[index]['playbook-name']+"\" value="+playbook_names[index]['playbook-name']+" /></td><td style=\"width: 60%;\"><label for=\"option_cus"+playbook_names[index]['playbook-name']+"\">"+playbook_names[index]['playbook-name']+"</label></td><td style=\"width: 25%;text-align: center\"></td></tr>";
                }
            
//            console.log(option_table_entry);
   ...
   const file = files[i];
    ...
    var parts = file.name.split('.');
		...
		process_playbook(file_json);
		  ...
		  console.log(JSON.stringify(file_json, null, '\t'));
		      process_playbook(file_json);
